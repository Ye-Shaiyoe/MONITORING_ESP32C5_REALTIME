...
const char*  host     = "sasa.id";
const String path     = "/kemendag/indo/indo/kirimdata5.php";
...
float tekanan        = 0.0;   // hPa — dari GY-801 BMP085
bool  bmpOK          = false; // true jika BMP085 berhasil diinit

String  flukeLine      = "";
bool    flukeDataReady = false;

unsigned long lastBmpRead    = 0;
const unsigned long BMP_INTERVAL = 5000; // baca tekanan tiap 5 detik
...
void updateLCD() {
  ...
  // ── Row 1: RH | Suhu | Tekanan ────────────────────────────────────────────
  // Format: "50.5|23.2|1013" — %.0f untuk tekanan agar muat 16 char
  if (bmpOK)
    snprintf(row, sizeof(row), "%.1f|%.1f|%.0f", kelembapan, suhu, tekanan);
  else
...
void readBMP() {
  if (!bmpOK) return;
  if (millis() - lastBmpRead < BMP_INTERVAL) return;
  lastBmpRead = millis();

  // readPressure() mengembalikan Pascal, bagi 100 untuk hPa
  float p = bmp.readPressure() / 100.0f;

  // Sanity check: tekanan atmosfer di permukaan laut ±200 hPa toleransi
  if (p < 700.0f || p > 1100.0f) {
    Serial.printf("[BMP] Tekanan tidak wajar: %.2f hPa, skip.\n", p);
    return;
  }

  tekanan = p;
  Serial.printf("[BMP] Tekanan: %.2f hPa\n", tekanan);
}
...
void processFluke() {
  ...
  // Filter Fluke — wajib dalam range
  bool flukeOK = (suhu       >= 10.0f && suhu       <= 35.0f) &&
                 (kelembapan >= 35.0f && kelembapan <= 90.0f);

  // Filter BMP085 — hanya diterapkan kalau sensor terpasang dan punya data
  bool bmpRangeOK = !bmpOK || (tekanan >= 800.0f && tekanan <= 1100.0f);

  if (flukeOK && bmpRangeOK) {
    HttpPayload payload = { suhu, kelembapan, tekanan };
    if (xQueueSend(httpQueue, &payload, 0) == pdTRUE)
      Serial.printf("[HTTP] Antri — T:%.1f RH:%.1f P:%.1f — next %lus\n",
                    suhu, kelembapan, tekanan, sendInterval / 1000);
    else
      Serial.println("[HTTP] Queue penuh, skip.");
  } else {
    Serial.printf("[Filter] Di luar range — T:%.1f°C RH:%.1f%% P:%.1fhPa — skip.\n",
...
void setup() {
  ...
  if (bmpOK) {
    ...
    Serial.printf("[BMP] Tekanan awal: %.2f hPa\n", tekanan);
  } else {
    Serial.println("[BMP] BMP085 tidak ditemukan! Cek wiring SDA/SCL & I2C address 0x77");
    Serial.println("[BMP] Sistem tetap jalan — tekanan = 0 (tidak dikirim ke filter)");
  }
