...
- [Fitur Utama & Inovasi Teknis](#-fitur-utama--inovasi-teknis)
  ...
  - [3. Auto-Recovery Bus I2C & Tampilan LCD Stabil](#3-auto-recovery-bus-i2c--tampilan-lcd-stabil)
  - [4. Validasi Data Sensor & HTTPS Resilient](#4-validasi-data-sensor--https-resilient)
...
### Latar Belakang
Sistem **Weather Station LAB K46** merupakan sistem pemantauan kondisi lingkungan laboratorium berbasis IoT secara real-time. Sistem ini membaca parameter cuaca (**Suhu / Temperature**, **Kelembapan / Relative Humidity**, dan **Tekanan Udara / Barometric Pressure**) dari sensor presisi tinggi (MHB-382SD / Fluke 1620A) lalu menampilkannya pada layar LCD 16x2 I2C serta mengirimkannya ke database web dashboard via HTTPS.
...
```mermaid
flowchart LR
    Sensor[Sensor MHB-382SD] -->|UART 9600 bps| ESP32[ESP32-C5 RISC-V]
...
3. Memperbaiki akurasi parsing paket serial sensor agar data suhu, kelembapan, dan tekanan tidak pernah tertukar.
4. Menerapkan mekanisme pemulihan otomatis (*auto-recovery*) pada bus I2C LCD dan koneksi jaringan.
5. Mempermudah replikasi sistem untuk berbagai laboratorium (Lab A hingga E dan Lab K46).
...
### 2. Wiring Sensor MHB-382SD (Komunikasi Serial 1 Arah)
...
Sensor cuaca mengirim 3 paket berurutan: **Kelembapan (RH)**, **Suhu (Temp)**, dan **Tekanan (Bar)**. Parsing berdasarkan urutan posisi sering membuat data tertukar saat mikrokontroler menyala di tengah transmisi.
...
void dispatchPaket(char* satuan, char* nilai) {
  ...
  // Simpan data & tandai valid
}
```

### 3. Auto-Recovery Bus I2C & Tampilan LCD Stabil
...
### 4. Validasi Data Sensor & HTTPS Resilient
...
├── ESP32-C5_WeatherStation_LAB_A.ino      # Versi deploy khusus Lab A
├── ESP32-C5_WeatherStation_LAB_B.ino      # Versi deploy khusus Lab B
├── ESP32-C5_WeatherStation_LAB_C.ino      # Versi deploy khusus Lab C
├── ESP32-C5_WeatherStation_LAB_D.ino      # Versi deploy khusus Lab D
├── ESP32-C5_WeatherStation_LAB_E.ino      # Versi deploy khusus Lab E
├── ESP32_C5_FIX_FLUKE_HYGROMETER.ino      # Varian untuk Hygrometer Fluke 1620A
...
Buka file target (contoh: `ESP32-C5_WeatherStation_LAB_K46-FIX.ino`), lalu sesuaikan konfigurasi jaringan jika diperlukan:
...
const String path    = "/path/ke/endpoint/kirimdata6.php";
```
...
Perangkat keras dan skema wiring dirancang **100% identik** untuk seluruh ruangan laboratorium. Anda hanya perlu mengganti baris `path` endpoint pada kode sebelum melakukan upload:
...
Berikut rangkuman kendala yang ditemukan selama riset migrasi dan solusinya:
